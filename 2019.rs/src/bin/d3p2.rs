...
fn main() -> io::Result<()>
{
    ...
    for (_k, v) in &grid
    {
        if v.val == 3
        {
            if v.steps[0] + v.steps[1] < answer.val
            {
                answer.val = v.steps[0] + v.steps[1];
                answer.x = v.x;
                answer.y = v.y;
            }
        }
...
fn place_wire(grid: &mut HashMap<String, Tile>, wire: &std::vec::Vec<Wire>, weight: u32)
{
    let mut x = 0;
    let mut y = 0;
    let mut step = 0;
    ...
    for leg in wire
    {
        for _ in 1..=leg.len
        {
            ...
            step += 1;
            place_dot(grid, x, y, weight, step);
        }
    }
}

fn place_dot(grid: &mut HashMap<String, Tile>, x: i32, y: i32, weight: u32, step: u32)
{
    ...
    if grid.contains_key(&key)
    {
        ...
        tmp.steps[weight as usize - 1] = step;

    } else {
        let mut tmp_steps : [u32; 2] = [0 ; 2];
        tmp_steps[weight as usize - 1] = step;
        grid.insert(key, Tile {val: weight, x: x, y: y, steps: tmp_steps});
    }
    // suggestion
